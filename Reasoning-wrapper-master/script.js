...
btn.addEventListener('click', () => {
  ...
  function renderWorkshopEditor() {
    ...
    (activeWorkshopMode.agents || []).forEach((a, idx) => {
      ...
      const isAdvanced = !!workshopAdvancedVisible[idx];

      const card = document.createElement('div');
      card.className = `workshop-card ${isExpanded ? 'expanded' : ''}`;
      
      let bodyHtml = '';
      if (isExpanded) {
        bodyHtml = 
          `<div class="workshop-card-body">` +
            `<label style="display:block;font-size:0.7rem;color:var(--text-muted);margin-bottom:4px;">NAME</label>` +
            `<input data-name="${idx}" value="${esc(a.name || '')}" style="width:100%;padding:8px;border-radius:6px;border:1px solid var(--border);background:var(--bg);color:var(--text);" placeholder="Agent Name" />` +
            `<label style="display:block;font-size:0.7rem;color:var(--text-muted);margin-top:10px;margin-bottom:4px;">PERSONALITY / SYSTEM TEXT</label>` +
            `<textarea data-persona="${idx}" rows="4" style="width:100%;padding:8px;border-radius:6px;border:1px solid var(--border);background:var(--bg);color:var(--text);" placeholder="Instructions for this agent...">${esc(a.persona || '')}</textarea>` +
            
            `<div class="workshop-advanced-toggle" data-advanced-toggle="${idx}">` +
              `<span>⚙</span> ${isAdvanced ? 'Hide Advanced Options' : 'Show Advanced Options'}` +
            `</div>` +
            
            (isAdvanced ? 
              `<div class="workshop-advanced-panel">` +
                `<label style="display:block;font-size:0.7rem;color:var(--text-muted);margin-bottom:4px;">ATTACHED FILES (multi-select)</label>` +
                `<select data-files="${idx}" multiple size="4" style="width:100%;padding:6px;border-radius:6px;border:1px solid var(--border);background:var(--bg);color:var(--text);">${fileOptionsHtml(a.files || [])}</select>` +
                `<label style="display:block;font-size:0.7rem;color:var(--text-muted);margin-top:10px;margin-bottom:4px;">TEMPERATURE (0.0 - 1.5)</label>` +
      ...
      card.innerHTML = 
        `<div class="workshop-card-header" data-header-index="${idx}">` +
          `<span class="workshop-chevron">➔</span>` +
          `<strong style="font-size:0.78rem;color:var(--text);">${esc(a.name || `Agent ${idx + 1}`)}</strong>` +
          `<span style="font-size:0.64rem;color:var(--text-muted);margin-left:6px;">(Agent ${idx + 1})</span>` +
          `<div class="workshop-card-actions">` +
            ...
            `<button class="hdr-btn" data-del="${idx}" title="Remove Agent" style="border-color:var(--primary);color:var(--primary);">Remove</button>` +
          `</div>` +
        `</div>` +
        bodyHtml;
    ...
    // Event listeners
    workshopAgents.querySelectorAll('[data-header-index]').forEach(el => {
      el.addEventListener('click', (e) => {
        // Prevent expanding/collapsing if clicking actions
        if (e.target.closest('.workshop-card-actions')) return;
        const i = Number(el.dataset.headerIndex);
        workshopExpandedIndex = (workshopExpandedIndex === i ? null : i);
        renderWorkshopEditor();
      });
    });

    workshopAgents.querySelectorAll('[data-advanced-toggle]').forEach(el => {
      el.addEventListener('click', () => {
        const i = Number(el.dataset.advancedToggle);
        workshopAdvancedVisible[i] = !workshopAdvancedVisible[i];
        renderWorkshopEditor();
      });
    });
    ...
    workshopAgents.querySelectorAll('[data-name]').forEach(el => {
      el.addEventListener('input', (e) => {
        ...
        const cardHeaderTitle = el.closest('.workshop-card')?.querySelector('.workshop-card-header strong');
        if (cardHeaderTitle) cardHeaderTitle.textContent = e.target.value || `Agent ${i + 1}`;
      });
    });

    workshopAgents.querySelectorAll('[data-persona]').forEach(el => {
      el.addEventListener('input', (e) => {
        const i = Number(el.dataset.persona);
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    try {
      const currentAgent = missionAgents.find(a => a.chat_id === store.currentId);
      if (currentAgent) {
        const sources = Array.isArray(currentAgent.sources) ? currentAgent.sources.filter(Boolean).slice(0, 12) : [];
        const pFiles = _projectFiles(currentAgent.project_id || missionSelectedProjectId)
          .slice(0, 10)
          .map(f => `FILE: ${f.name}\n${String(f.content || '').slice(0, 2400)}`)
          .join('\n\n');
        const profileBlock =
          ...
          `instructions=${(currentAgent.instructions || '').slice(0, 1200)}\n` +
          `memory_notes=${(currentAgent.memory_notes || '').slice(0, 2400)}\n` +
          `${sources.length ? `sources=\n- ${sources.join('\n- ')}` : ''}\n` +
          `[/AGENT_PROFILE]`;

        const isManager = currentAgent.is_project_head || currentAgent.mode === 'project_manager';
        const subagents = (currentAgent.project_id && missionAgents) ? missionAgents.filter(a => a.project_id === currentAgent.project_id && a.id !== currentAgent.id).map(a => a.name) : [];
        
        let sysRules = '';
        if (currentAgent.project_id || missionSelectedProjectId) {
          sysRules += `\n\n[FILE SYSTEM CAPABILITY]\nYou can create and edit project files. To do so, output a markdown codeblock starting exactly with \`\`\`file:filename.ext\n[content]\n\`\`\`.\nThe system will automatically save it. Use this to update VISION.md, SHARED_CONTEXT.md, or SCOPE.md.\n`;
        }
        if (isManager && subagents.length > 0) {
          sysRules += `[TEAM MANAGER CAPABILITY]\nYou lead this project. Your available subagents are: ${subagents.join(', ')}.\nTo delegate a task, output a codeblock starting exactly with \`\`\`agent:AgentName\n[task commands]\n\`\`\`. The system will automatically forward your command to them.\n`;
        } else if (!isManager && currentAgent.project_id) {
          sysRules += `[TEAM SYNC]\nYou are a focused subagent. Perform your assignment, update relevant logic via files, and explain changes to your manager in your reply.\n`;
        }
        ...
        const mode = currentAgent.mode || 'reasoning_fast';
        ...
        if (res.ok && data?.reply) {
          let replyText = data.reply;
          let filesEdited = 0;
          let projId = currentAgent.project_id || missionSelectedProjectId;
          let files = projId ? _projectFiles(projId) : null;
          ...
          currentAgent.last_status = 'active';
          currentAgent.last_command_at = Date.now();
          saveMissionAgents(missionAgents);
          renderMissionProjects();
          ...
          while ((agentMatch = agentRegex.exec(replyText)) !== null) {
            let subName = String(agentMatch[1]).trim().toLowerCase();
            let subTask = agentMatch[2].trim();
            let targetAgent = (missionAgents || []).find(a => (a.project_id === currentAgent.project_id || !currentAgent.project_id) && String(a.name || '').toLowerCase() === subName && a.id !== currentAgent.id);
            if (targetAgent) {
              showToast(`Delegating task to ${targetAgent.name}...`, 'info');
              sendCommandToAgent(targetAgent, subTask).catch(e => console.error('Subagent failed', e));
            } else {
          ...
          currentAgent.last_status = 'error';
          saveMissionAgents(missionAgents);
